...
func ParseFlags() (config Settings)  { 
	...
	flag.DurationVar(&config.MINIMAL_LAP_TIME_DURATION, "minimal-lap-time", 45*time.Second, "Minimal lap time duration. Results smaller than this duration would be considered wrong. Valid time units are: 's' (second), 'm' (minute), 'h' (hour)." )
